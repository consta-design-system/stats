...
Если в компоненте используется иконка, которой нет в UI-ките, ее можно создать через `@consta/uikit/createIcon`. [Подробнее о создании иконок](http://uikit.gizeasy.ru/?path=/docs/components-icons--playground)
